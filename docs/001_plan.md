...
### 5.4 transfer_codes（データ引き継ぎコード）

| カラム       | 型          | 制約             | 説明                                                                                     |
| ------------ | ----------- | ---------------- | ----------------------------------------------------------------------------------------- |
| `id`         | serial      | PK               | 内部 ID                                                                                   |
| `code`       | text        | unique, not null | 引き継ぎコード（8 桁。`0/O`・`1/I/L` 等の紛らわしい文字を除いた英数字から生成）           |
| `payload`    | jsonb       | not null         | 匿名ユーザー ID・投票済み/スキップ済み/既読/お気に入りの各 oneshot ID 一覧                |
| `created_at` | timestamptz | not null         | 発行日時                                                                                  |
| `expires_at` | timestamptz | not null         | 有効期限（発行から 24 時間）                                                              |

- 端末間で localStorage 上の記録（匿名 ID・投票/スキップ/既読/お気に入り）を移行するための使い捨てコード（「6.5 データ引き継ぎ」参照）
- 引き換え（redeem）時に有効期限内であれば即座に物理削除する。未使用のまま期限切れになったコードはバッチで定期的に物理削除する（「8.5 データ引き継ぎコードの削除」参照）
...
- `/about` ページ内の機能。投票/スキップ/既読/お気に入りの記録は localStorage のみで管理しているため、
  端末を変えると引き継げない。その記録を別端末へ移行する手段として提供する
- 「引き継ぎコードを発行する」操作で 8 桁の引き継ぎコード（紛らわしい文字を除いた英数字）を発行する。
  コードの有効期限は 24 時間
- 別端末でコードを入力すると、「上書き（overwrite）」「統合（merge）」のいずれかのモードで
  現在の端末のローカルデータへ反映する
- コードは引き換え（redeem）時に即座に無効化（物理削除）される使い捨て方式（API 仕様は「9. API 仕様」参照）
- 認証は行わない。コードを知っている端末であれば誰でも引き換え可能なため、有効期限を短く設定している

### 6.6 PWA（インストール可能化・オフライン対応）

- Web App Manifest（`display: standalone`）により、ホーム画面への追加・インストールに対応する
- Service Worker はナビゲーション（HTML ページ遷移）リクエストのみを対象とし、
  オフライン時は `/offline` ページへフォールバックする。投票 API や ISR/CDN によってキャッシュされる
  静的アセットには一切介入せず、既存のキャッシュ戦略と競合しないようにする

### 6.7 RSS 配信

- `/rss.xml` で新着の読み切り漫画を RSS フィードとして配信する
...
URL 収集・詳細取得と合わせて、バッチ実行のたびに期限切れ（発行から 24 時間経過）のまま
未使用の `transfer_codes` を物理削除する。有効期限内かどうかの正しさ自体は
引き換え（redeem）API 側の判定で担保しているため、このバッチはストレージ節約が目的。
...
データ引き継ぎコードを発行する（「6.5 データ引き継ぎ」参照）。
...
```json
{
  ...
  "votedOneshotIds": [1, 2],
  "skippedOneshotIds": [3],
  "readOneshotIds": [1, 2, 3],
  "favoriteOneshotIds": [4]
}
```
...
| ステータス               | 条件                                                 |
| ------------------------- | ----------------------------------------------------- |
| `201 Created`              | コードを発行した（`code` / `expiresAt` を返す）        |
| `400 Bad Request`          | ボディ不正                                             |
| `429 Too Many Requests`    | 同一 IP からの連続投稿によるレート制限超過              |

### POST /api/data-transfer/[code]

発行済みの引き継ぎコードを引き換える（redeem）。
...
`mode` は `overwrite`（上書き）または `merge`（統合）のいずれか。

レスポンス:

| ステータス               | 条件                                                 |
| ------------------------- | ----------------------------------------------------- |
| `200 OK`                   | 引き換え成功。`payload`（保存されていた各 ID 一覧）と `mode` を返す |
| `400 Bad Request`          | ボディ不正                                             |
| `404 Not Found`             | コードが存在しない、または有効期限切れ                  |
| `429 Too Many Requests`    | 同一 IP からの連続投稿によるレート制限超過              |

- 引き換え成功時、コードは即座に物理削除する（使い捨て）
