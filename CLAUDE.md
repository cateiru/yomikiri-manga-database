...
- `apps/batch` — 各サービスをクロールしてデータを収集するバッチ（さくら VPS 上で Docker コンテナとして systemd timer により毎日 00:05・12:05・18:05（JST）に実行、Mackerel で監視）
...
外部サイト遷移のため正確な読了は検知できず、「外部遷移してから 10 秒以上経って戻ってきた」ことをヒューリスティックに読了とみなす（`apps/web/src/lib/readDetection.ts`）。判定・スキップ状態は localStorage で管理する（サーバー側に状態を持たない）。
